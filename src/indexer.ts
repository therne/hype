...
export interface Indexer<I, O> {
  id: string;
  logFinders: ReturningLogFinderMapper<I>[];
  ...
  async start() {
    for await (const block of this.dataSource.blocks()) {
      ...
      for (const indexerDefinition of this.indexers) {
        await this.runIndexer(block, indexerDefinition);
      }
      this.emit('endBlockProcess', block);
    }
  }

  private async runIndexer<I, O>(block: Block, { id, logFinders, indexer: indexerFn, postIndexerHook }: Indexer<I, O>) {
    ...
    for (const input of inputs) {
      const output = await indexerFn(input, block);
      outputs.push(...output);
    }
